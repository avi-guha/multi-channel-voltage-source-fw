...
constexpr const char* kLogTag = "SharedSpiBus";
...
bool SharedSpiBus::begin(int sclk_pin, int miso_pin, int mosi_pin,
  ...
  if ((init_result != ESP_OK) && (init_result != ESP_ERR_INVALID_STATE)) {
    platform::logError(kLogTag, "spi_bus_initialize failed: %d", static_cast<int>(init_result));
    return false;
  }

  if (device_count > kMaxDevices) {
    platform::logError(kLogTag, "too many SPI devices: %u", static_cast<unsigned>(device_count));
    return false;
  }
  ...
  for (size_t i = 0; i < device_count; ++i) {
    ...
    if (handle == nullptr) {
      platform::logError(kLogTag, "missing device handle for %s", devices[i].name);
      return false;
    }
...
bool SharedSpiBus::beginSession(const SpiDeviceConfig& device) {
  if (current_device_ != nullptr) {
    platform::logError(kLogTag, "SPI session already active");
    return false;
  }

#if defined(ARDUINO)
  SPISettings settings(device.clock_hz, MSBFIRST, static_cast<uint8_t>(device.mode));
  ...
  if (current_handle_ == nullptr) {
    platform::logError(kLogTag, "no SPI handle registered for %s", device.name);
    return false;
  }
...
bool SharedSpiBus::transfer(const uint8_t* tx_data, uint8_t* rx_data, size_t length_bytes) {
  ...
  for (size_t i = 0; i < length_bytes; ++i) {
    const uint8_t tx_byte = (tx_data != nullptr) ? tx_data[i] : 0xFF;
    const uint8_t rx_byte = spi_->transfer(tx_byte);
    if (rx_data != nullptr) {
      rx_data[i] = rx_byte;
    }
  }
  ...
  if ((tx_data == nullptr) && (length_bytes > sizeof(dummy_tx))) {
    platform::logError(kLogTag, "dummy transfer too large: %u", static_cast<unsigned>(length_bytes));
    return false;
  }
...
#if defined(ARDUINO)
  digitalWrite(current_device_->cs_pin, HIGH);
  spi_->endTransaction();
#else
