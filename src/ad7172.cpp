...
constexpr const char* kLogTag = "AD7172-2";
...
bool Ad7172_2::begin() {
  if (!reset()) {
    platform::logError(kLogTag, "ADC reset failed");
    return false;
  }
  platform::delayMs(2);
  ...
  if (!readRegister(kRegId, id_value, 2)) {
    platform::logError(kLogTag, "unable to read ID register");
    return false;
  }
  ...
  for (uint8_t channel = 0; channel < 4; ++channel) {
    ...
    if (!writeRegister(static_cast<uint8_t>(kRegCh0 + channel), channel_register, 2)) {
      platform::logError(kLogTag, "failed to configure channel %u", static_cast<unsigned>(channel));
      return false;
    }
  }

  platform::logInfo(kLogTag, "initialized, ID=0x%04X", static_cast<unsigned>(id_value));
  return true;
}

float Ad7172_2::readADCChannel(uint8_t channel) {
  if (channel > 3U) {
    platform::logError(kLogTag, "invalid channel index %u", static_cast<unsigned>(channel));
  ...
  platform::logError(kLogTag, "timed out waiting for channel %u", static_cast<unsigned>(channel));
  return std::numeric_limits<float>::quiet_NaN();
}
